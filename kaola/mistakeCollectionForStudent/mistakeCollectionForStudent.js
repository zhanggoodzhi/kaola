define([
    'angular', 'ng-table', '/web/components/directives/mistakeCollection/question.js', 'angular-ui-sortable'
], function (angular) {
    'use strict';
    ...
    */
    angular
        ...
        .controller('MistakeCollectionForStudentCtrl', [
            ...
            'AuthService',
            'Constants',
            'MistakeCollectionForStudentService',
            '$state',
            'ShowService',
            '$timeout',
            'CommonBusinessService',
            function ($scope, $rootScope, $stateParams, NgTableParams, AuthService, Constants, PaperDesignService, $state, ShowService, $timeout, CommonBusinessService) {
                ...
                console.log("UrlConfig:", $scope.UrlConfig);
                ...
                //#endregion #region data structure define
                ...
                $scope.CommonOrderBy = 'DESC'; //综合的排序方式
                $scope.ScoreRateOrderBy = 'DESC'; //得分率的排序方式
                $scope.AnswerCountOrderBy = 'DESC'; //答题次数的排序方式
                ...
                //#endregion #region QuestionData Cache
                                ...
                                if (findCache == false) {
                                    ...
                                    $scope
                                        ...
                                        .push({
                                            ...
                                            AnswerList: newQuestion.AnswerList,
                                            CurrentSelectedTask: newQuestion.AnswerList[0]
                                        })
                                }
                            }
                        }
                ...
                $scope.ChooseTask = function (task, data) {
                    data.CurrentSelectedTask = task;
                }
                ...
                //#endregion #region get areaType list
                $scope.AreaTypeList = [];
                ...
                $scope.AreaTypeChoosed = function (a) {
                    if (a.AreaType == $scope.CurrentAreaTypeChoosed) {
                        return;
                    }
                ...
                //#endregion #region search function
                $scope.SearchText = function () {
                    ...
                    //清空显示的数据
                    $scope
                        .PaperDesignDisplayData
                        .Data
                        .splice(0, $scope.PaperDesignDisplayData.Data.length);
                ...
                $scope.Search = function () {
                    ...
                    if ($scope.CurrentTabID == 0) {
                        $scope.Filter.CommonOrderBy = $scope.CommonOrderBy;
                        $scope.Filter.ScoreRateOrderBy = '';
                        $scope.Filter.AnswerCountOrderBy = '';
                    }
                    if ($scope.CurrentTabID == 1) {
                        $scope.Filter.ScoreRateOrderBy = $scope.ScoreRateOrderBy;
                        $scope.Filter.CommonOrderBy = '';
                        $scope.Filter.AnswerCountOrderBy = '';
                    }
                    if ($scope.CurrentTabID == 2) {
                        $scope.Filter.AnswerCountOrderBy = $scope.AnswerCountOrderBy;
                        $scope.Filter.CommonOrderBy = '';
                        $scope.Filter.ScoreRateOrderBy = '';
                    }
                    ...
                    PaperDesignService
                        ...
                        .then(function (result) {
                            ...
                            if (!data || data === 'null') {
                                ...
                                angular.element('#no-resource').show();
                                return;
                            }
                            angular.element('#no-resource').hide();
                ...
                //#endregion #region  choose/unchoose question
                ...
                $scope.UnChooseAllQuestion = function () {
                    ...
                    $scope
                        .openCommonModalDialog("确认", "您确定要清空所有已选试题吗?", function () {
                            var allQuestions = [];
                            if ($scope.PaperDesignChoosedData.Data != undefined && $scope.PaperDesignChoosedData.Data.length > 0) {
                                for (var i = 0; i < $scope.PaperDesignChoosedData.Data.length; i++) {
                                    var areaItem = $scope.PaperDesignChoosedData.Data[i];
                ...
                $scope.UpdatePaperDesignChoosedDataProperty = function (checkMins) {
                    ...
                    if (!checkMins) {
                        if ($scope.PaperDesignChoosedData.PaperTotalSeconds > $scope.MAX_PAPER_TOTAL_SECONDS) {
                            ...
                            angular
                                .element('#PaperTotalSecondsInfoDialog')
                ...
                $scope.ChangeScore = function (q, areaType) {
                    console.log(q);
                    //1. 单小题, Contents.length=1 分数= ExtProperty.Contents[0].Score

                    //2. 多小题, Contents.length>1 分数= ExtProperty.Contents[i].Score
                    //3. 跟读与模仿16 分数= question.QuestionScore
                    // 4. 听后记录并转述信息25 第一节分数 = ExtProperty.Contents[i].Score , i !=last index 第二节分数 =
                    // ExtProperty.Contents[i].Score,  i=last index
                    // 5. 信息转述与询问26 第一节分数 = ExtProperty.Contents[0].Score , i =0 第二节分数 =
                    // ExtProperty.RelationQuestionModel.ExtProperty.Contents[i].Score
                    ...
                    if (areaType == 26) {
                        ...
                        if (q.ExtProperty.RelationQuestionModel != undefined && q.ExtProperty.RelationQuestionModel.ExtProperty.Contents != undefined) {
                ...
                //#endregion #region get init param
                ...
                //#endregion #region adjust UI for scorll and float panel
                $scope.InitAreaTypeFinished = function () {

                    // console.log("areaType in header filter repeat finished ,
                    // InitAreaTypeFinished");
                    ...
                    $timeout(function () {
                        ...
                        var areaTypeFilterTop = $('.areatype-filter')
                            .offset()
                            .top;
                        ...
                        var areaTypeFilterLeft = $('.areatype-filter')
                            .offset()
                            .left;
                ...
                $scope.InitAreaTypeFinishedForScroll = function (showFlag) {
                    // console.log("areaType in Scroll filter repeat finished ,
                    // InitAreaTypeFinishedForScroll"); init scroll part
                ...
                $scope.ResizeMyPaperContentMaxHeight = function () {
                    // console.log("myPaper areaItem repeat finished,
                    // ResizeMyPaperContentMaxHeight"); var windowHeight = $(window).height();
                    // console.log("windowHeight:" + windowHeight); var myPaperDivTop =
                    // $('.mypaper-container').position().top; console.log("myPaperDivTop:" +
                    // myPaperDivTop); var myPaperDivContentMaxHeight = windowHeight - myPaperDivTop
                    // - 200; console.log("myPaperDivContentMaxHeight:" +
                    // myPaperDivContentMaxHeight); $('.mypaper-questions').css('max-height',
                    // myPaperDivContentMaxHeight); var actualMyPaperContentDivHeight =
                    // $('.mypaper-questions .mCSB_container').height();
                    // console.log("actualMyPaperContentDivHeight:" +
                    // actualMyPaperContentDivHeight); if (actualMyPaperContentDivHeight >
                    // myPaperDivContentMaxHeight) {    $('.mypaper-questions').css('height',
                    // actualMyPaperContentDivHeight); } else {
                    // $('.mypaper-questions').css('height', ''); }
                    // $(".mypaper-questions.custom-scrollbar").mCustomScrollbar();
                ...
                $(window).on('scroll', function () {
                    ...
                    if (containerElement) {
                        var containerTop = containerElement
                            .offset()
                            .top;
                ...
                //#endregion  adjust areaType UI #region  save and preview
                $scope.ShowSaveDialog = function () {

                    // 检查小题的分数, 要求同一题型内,小题分数一致 if ($scope.CheckContentScore() == false) {    return;
                    // }

                    $scope.PaperName = $scope.PaperDesignChoosedData.PaperName;
                    angular
                        .element('#SavePaperDialog')
                        .modal({backdrop: 'static', keyboard: false});
                }
                    ...
                    PaperDesignService
                        ...
                        .then(function (response) {
                            ...
                            if (success) {
                                ...
                                PaperDesignService
                                    .SavePaperDesignTempData($scope.PaperDesignChoosedData)
                                    .then(function (response) {
                    ...
                    PaperDesignService
                        ...
                        .then(function (response) {
                            ...
                            if (response.data.Success) {
                                ...
                                angular
                                    .element('#PaperGenerateResultDialog')
                                    .modal({backdrop: 'static', keyboard: false});
                            }
                ...
                $scope.StartGeneratePaperProgress = function () {
                    ...
                    angular
                        .element('#GeneratePaperProgressDialog')
                        .modal({backdrop: 'static', keyboard: false});
                            ...
                            if (areaItem.AreaType != 25 && areaItem.AreaType != 26 && areaItem.AreaType != 16) {
                                ...
                                for (var qIndex = 0; qIndex < questions.length; qIndex++) {
                                    ...
                                    for (var cIndex = 0; cIndex < contents.length; cIndex++) {
                                        ...
                                        if (score === undefined || score === null || score === 0 || contents[cIndex].Score === undefined || contents[cIndex].Score === null || contents[cIndex].Score === 0) {
                                            checkValid = false;
                                            $rootScope.openCommonErrorDialog("错误", "题型[" + areaItem.AreaTitle + "]中小题分数不能为空或者零");
                                            return checkValid;

                                        }
                                        if (score === undefined || score === null || contents[cIndex].Score === undefined || contents[cIndex].Score === null) {
                                            scoreCount++;
                                        }
                                    }
                                }
                            ...
                            if (areaItem.AreaType == 16) {
                                ...
                                for (var qIndex = 0; qIndex < questions.length; qIndex++) {
                                    ...
                                    if (contents.length > 0) {
                                        ...
                                        if (score === undefined || score === null || contents[0].Score === undefined || contents[0].Score === null || score !== contents[0].Score) {
                                            scoreCount++;
                                        }
                                    }
                                }
                            ...
                            if (areaItem.AreaType == 25) {
                                ...
                                for (var qIndex = 0; qIndex < questions.length; qIndex++) {
                                    ...
                                    for (var cIndex = 0; cIndex < contents.length - 1; cIndex++) {
                                        ...
                                        if (score === undefined || score === null || contents[cIndex].Score === undefined || contents[cIndex].Score === null || score !== contents[cIndex].Score) {
                                            scoreCount++;
                                        }
                                    }
                                    if (scoreCount > 1) {
                ...
                //#endregion #region drag and drop by angular-ui-sortable
                        ...
                        for (var qIndex = 0; qIndex < qElementList.length; qIndex++) {
                            ...
                            if (arr.length >= 4) {
                                ...
                                if (areaType == dragAreaType) {
                                    ...
                                    newQIndexList.push({index: currentQuestionIndex, areaType: areaType, questionID: qID});
                                }
                ...
                //#endregion #region miscellaneous methods
                $scope.PaperDesignTabChange = function (tabID) {

                    $scope.CurrentTabID = tabID;
                    if (tabID == 0) {
                        console.log("CommonOrderBy:", $scope.CommonOrderBy);
                        $scope.CommonOrderBy = ($scope.CommonOrderBy == 'DESC'
                            ? 'ASC'
                            : 'DESC');
                    }
                    if (tabID == 1) {
                        console.log("ScoreRateOrderBy:", $scope.ScoreRateOrderBy);
                        $scope.ScoreRateOrderBy = ($scope.ScoreRateOrderBy == 'DESC'
                            ? 'ASC'
                            : 'DESC');
                    }
                    if (tabID == 2) {
                        console.log("AnswerCountOrderBy:", $scope.AnswerCountOrderBy);
                        $scope.AnswerCountOrderBy = ($scope.AnswerCountOrderBy == 'DESC'
                            ? 'ASC'
                            : 'DESC');
                    }
                ...
                //#endregion #region  search text highlight
...
* Services
*/
        .service('MistakeCollectionForStudentService', function ($http, Constants) {
            var self = this;

            self.GetAreaType = function () {
                return $http.get(GLOBAL_API_URL + 'api/mistakeforstudent/areatypelist');
            }
            self.Search = function (filter) {
                return $http.post(GLOBAL_API_URL + 'api/mistakeforstudent/search', filter);
            }
        })
