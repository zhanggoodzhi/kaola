define([
    'angular', 'echarts', 'components/directives/questionDetail/teacher-report/teacher-questionDetail.js', 'ng-table', 'jquery-scrollbar', 'jquery-mousewheel'
    ...
    angular
        ...
        .controller('TeacherReportTaskDetailCtrl', [
            ...
            'Audio',
            function ($scope, $rootScope, $state, ReportService, NgTableParams, $stateParams, $timeout, Constants, CommonBusinessService, Audio) {
                $scope.Constants = {
                ...
                var selectData = function (classId, paperId) {
                    var data;
                    for (var i = 0; i < REPORT_DATA.teacher.length; i++) {
                        var teacherItem = REPORT_DATA.teacher[i];
                        if (classId == teacherItem.classId && paperId == teacherItem.paperId) {
                            data = teacherItem;
                        }
                    }
                    if (!data) {
                ...
                $scope.LoadTaskData = function (taskID, classID, paperID) {
                    ...
                    // { });
                    var result = {
                        data: selectData(classID, paperID).report_TaskListForTeacher
                    }
                ...
                $scope.LoadTaskStudentData = function (taskID, classID, studentFilter, paperID) {
                    ...
                    // ReportService.GetReport_TaskStudentForTeacher(queryParam).then(function
                    // (result) { });
                    var _result = {
                        data: selectData(classID, paperID || '').report_TaskStudentForTeacher
                    }
                    var result = {
                        data: []
                    };
                    for (var i = 0; i < _result.data.length; i++) {
                        if (_result.data[i].StudentName.indexOf(studentFilter) !== -1 || _result.data[i].StudentNumber.indexOf(studentFilter) !== -1) {
                            result
                                .data
                                .push(_result.data[i]);
                        }
                ...
                $scope.LoadTaskSummaryData = function (taskID, classID) {
                    ...
                    $timeout(function () {
                        ...
                        // ReportService.GetReport_TaskSummaryForTeacher(queryParam).then(function
                        // (result) { }, function (err) {     $scope.AllChart.hideLoading();
                        // $scope.ListeningChart.hideLoading();     $scope.SpeakingChart.hideLoading();
                        //    $scope.SetLoadChartDataError('allChart');
                        // $scope.SetLoadChartDataError('listeningChart');
                        // $scope.SetLoadChartDataError('speakingChart'); });
                        var result = {
                            data: selectData($scope.CurrentClassID, $scope.CurrentPaperID).report_TaskSummaryForTeacher
                        }
                    ...
                    if (data.ScorePercentList != undefined && data.ScorePercentList.length > 0) {
                        ...
                        for (var i = 0; i < data.ScorePercentList.length; i++) {
                            ...
                            if (studentCount != undefined && studentCount >= 0) {
                                pieSeriesData.push({ extData: d, value: studentCount, name: d.Desc });
                        ...
                        // console.log("xAixsData", xAixsData); console.log("legendData", legendData);
                        // console.log("lineSeriesData", lineSeriesData); console.log("barSeriesData",
                        // barSeriesData); console.log("pieSeriesData", pieSeriesData);
                ...
                $scope.LoadTaskPaperAnalysisData = function (taskID, classID, paperID) {
                    $timeout(function () {
                        ...
                        // ReportService.GetTaskPaperAnalysisDataForTeacher(taskID, classID,
                        // paperID).then(function (result) { }, function (error) {     if
                        // ($scope.EnableKnowledgePointAnalysisBarChart) {
                        // $scope.KnowledgePointAnalysisBarChart.hideLoading();
                        // $scope.SetLoadChartDataError('analysis_barChart');     } })
                        var result = {
                            data: selectData(classID, paperID).report_TaskPaperAnalysisDataForTeacher
                        }
                        var d = result.data;
                        ...
                        if ($scope.EnableKnowledgePointAnalysisBarChart) {
                            $scope
                                .KnowledgePointAnalysisBarChart
                                .hideLoading();
                            //知识点柱状图
                ...
                $scope.ViewAreaScoreDetail = function (la, $event) {
                    ...
                    angular
                        .element("#areaAnalysisChartDialog")
                ...
                $scope.recursePlay = function (audioPathArr) {
                    if (!audioPathArr.length) {
                        ...
                        Audio.play(audioPathArr[0], function () {
                            ...
                            if (item.ScoreScattergramType == dataType) {
                                ...
                                if (ScoreDetailList != undefined && ScoreDetailList.length > 0) {
                                    for (var dIndex = 0; dIndex < ScoreDetailList.length; dIndex++) {
                                        ...
                                        if (hideZero == true && dItem.Count <= 0)
                                            continue;

                                        legendData.push(dItem.ScoreDisplayText);
                                        pieSeriesData.push({ value: dItem.Count, name: dItem.ScoreDisplayText })
                                    }
                                }
                            }
                        }
                ...
                //miscellaneous methods 展开/收缩
                $scope.collapseToggle = function (d) {
                    if (d.CollapseState != undefined) {
                        d.CollapseState = d.CollapseState == 0
                            ? 1
                            : 0;
                    }
                }
