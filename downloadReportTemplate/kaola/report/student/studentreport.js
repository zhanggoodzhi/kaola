define(['angular', 'echarts','components/directives/questionDetail/student-report/student-questionDetail.js', 'ng-table', 'components/directives/questionDetail/student-report/student-questionDetail.js'], function (angular, echarts) {
    'use strict';
    /*
     * Controllers
     */
    angular.module('app')
        .controller('StudentReportTaskDetailCtrl', ['$scope', '$rootScope', '$state', 'ReportService', 'NgTableParams', '$stateParams', '$timeout', 'Constants', 'CommonBusinessService', function ($scope, $rootScope, $state, ReportService, NgTableParams, $stateParams, $timeout, Constants, CommonBusinessService) {
            $scope.Constants = {
            ...
            var report_data;
            // 匹配学生数组中的一项数据
            for (var i = 0; i < REPORT_DATA.student.length; i++) {
                if (REPORT_DATA.student[i].report_TaskAreaDataForStudent.StudentNumber == $stateParams.studentNumber) {
                    report_data = REPORT_DATA.student[i];
                }
            }
            //如未匹配到，默认选择第一个数据
            if (!report_data) {
                report_data = REPORT_DATA.student[0];
            }
            ...
            $scope.LoadStudentAreaData = function (taskID, studentNumber) {
                ...
                // });
                var result = {
                    data: report_data.report_TaskAreaDataForStudent
                }
            ...
            $scope.GetTaskPaperAnalysisDataForStudent = function (taskID, studentNumber, paperID) {
                ...
                // }, function (error) {})
                var result = {
                    data: report_data.report_TaskPaperAnalysisDataForStudent
                }
            ...
            $scope.LoadStudentTaskData = function (taskID, studentNumber) {
                ...
                // });
                var result = {
                    data: report_data.report_TaskListForStudent
                }
