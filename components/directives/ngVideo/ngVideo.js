define(['angular', 'videojs'], function (angular, videojs) {
    'use strict'
    angular.module('ngVideoModule', []).directive('ngVideo', function ($compile, $timeout) {
        return {
            restrict: 'AE',
            transclude: true,
            scope: {
            ...
            link: function ($scope, $element, $attrs) {
                ...
                $scope.StopVideoPlay = function () {
                    if ($scope.VideoPlaying) {
                        ...
                        player = null;
                        //dom
                        var videoHtml = '<video id=' + $scope.data.id + ' class="video-js vjs-default-skin vjs-big-play-centered"  width="100%"  controls></video>';

                        var elementID = '#ng-video-directives_' + $scope.data.id;

                        angular.element(elementID).html(videoHtml);//将模板内容重新插入根节点
                    }
                }
            ...
            template: '<video id="{{data.id}}" class="video-js vjs-default-skin vjs-big-play-centered"  width="100%"  controls></video>'
        }
    })
});
